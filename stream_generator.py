...
class GstStreamGenerator:
    def __init__(self, 
        ...
        # Éléments GStreamer
        self.pipelines = {"left": None, "right": None}
        self.main_loop = None
    ...
    def _create_klv_packet(self, filename, timestamp):
        ...
        Returns:
            bytes: Paquet KLV formaté
        """
        # Création d'un faux message protobuf comme chaîne
        klv_content = f'image_index: 1\nfilename: "{os.path.basename(filename)}"\nsession_name: "{self.session_name}"\ntimestamp: {timestamp}\n'
        
        # Key - 16 octets (une UUID simplifiée pour cet exemple)
        key = b'\x06\x0E\x2B\x34\x01\x01\x01\x01\x0D\x01\x02\x01\x01\x01\x01\x00'
        
        # Length - 4 octets, taille du contenu
        length = len(klv_content.encode('utf-8'))
        length_bytes = struct.pack('>I', length)  # Big-endian (format réseau)
        
        # Value - contenu du message
        value = klv_content.encode('utf-8')
        
        # Assembler le paquet KLV
        klv_packet = key + length_bytes + value
        
        return klv_packet
    
    def _create_ts_packet(self, klv_packet):
        """
        Crée un paquet de transport MPEG-TS contenant les données KLV
        """
        # En-tête MPEG-TS (4 octets)
        sync_byte = 0x47  # Sync byte pour MPEG-TS
        transport_error_indicator = 0
        payload_unit_start_indicator = 1
        transport_priority = 0
        pid = 0x1FFF  # PID pour données privées
        transport_scrambling_control = 0
        adaptation_field_control = 1  # Payload only
        continuity_counter = 0  # À incrémenter à chaque paquet
        
        # Premier octet
        header_byte1 = sync_byte
        
        # Deuxième octet
        header_byte2 = ((transport_error_indicator & 0x01) << 7) | \
                       ((payload_unit_start_indicator & 0x01) << 6) | \
                       ((transport_priority & 0x01) << 5) | \
                       ((pid >> 8) & 0x1F)
        
        # Troisième octet
        header_byte3 = pid & 0xFF
        
        # Quatrième octet
        header_byte4 = ((transport_scrambling_control & 0x03) << 6) | \
                       ((adaptation_field_control & 0x03) << 4) | \
                       (continuity_counter & 0x0F)
        
        # En-tête complet
        ts_header = bytes([header_byte1, header_byte2, header_byte3, header_byte4])
        
        # On crée un paquet de 188 octets (taille standard MPEG-TS)
        # On remplit avec des 0xFF si nécessaire
        payload_size = 184  # 188 - 4 octets d'en-tête
        
        # Si les données KLV sont plus grandes que ce qui peut tenir dans un paquet
        if len(klv_packet) > payload_size:
            # On tronque ou on pourrait implémenter la fragmentation sur plusieurs paquets
            klv_data = klv_packet[:payload_size]
        else:
            # On ajoute du padding
            padding_size = payload_size - len(klv_packet)
            klv_data = klv_packet + bytes([0xFF] * padding_size)
        
        # Paquet MPEG-TS final
        ts_packet = ts_header + klv_data
        
        return ts_packet
    
    def _start_tcp_server(self):
        """Démarre le serveur TCP pour les données KLV"""
        try:
            ...
            logger.info(f"Serveur TCP démarré sur {self.tcp_host}:{self.tcp_port}")
            
            while self.running:
                client_socket, addr = self.tcp_server_socket.accept()
                logger.info(f"Nouvelle connexion TCP: {addr}")
                self.tcp_client_sockets.append(client_socket)
        except Exception as e:
            logger.error(f"Erreur dans le serveur TCP: {e}")
        finally:
            if self.tcp_server_socket:
                self.tcp_server_socket.close()
    
    def _send_tcp_data(self, klv_packet):
        """Envoie des données KLV via TCP"""
        for client in list(self.tcp_client_sockets):
            try:
                client.sendall(klv_packet)
            except Exception as e:
                logger.warning(f"Erreur lors de l'envoi TCP: {e}")
                self.tcp_client_sockets.remove(client)
    ...
    def _setup_gstreamer_pipeline(self, side):
        ...
        # Utiliser des queues plus robustes et contrôler le flux
        pipeline_str = (
            ...
            # Branche SRT avec meilleure gestion de queue
            f"t. ! queue max-size-buffers=2 leaky=downstream ! "
            f"rtpjpegpay ! srtserversink uri=srt://{self.srt_host}:{port}/ "
            f"wait-for-connection=false latency=1000 async=false "

            # Branche locale avec meilleure gestion
        ...
        logger.info(f"Pipeline {side} démarrée avec succès")
        return None
        ...
        while self.running and num_pairs > 0:
            for i in range(num_pairs):
                ...
                try:
                    ...
                    # Création des paquets MPEG-TS contenant les KLV
                    left_ts = self._create_ts_packet(left_klv)
                    right_ts = self._create_ts_packet(right_klv)
                    
                    # Envoi des données KLV via TCP (en format MPEG-TS)
                    self._send_tcp_data(left_ts)
                    time.sleep(0.01)  # Petit délai entre les envois
                    self._send_tcp_data(right_ts)
    ...
    def start(self):
        ...
        try:
            ...
            for side, pipeline in self.pipelines.items():
                ...
                logger.info(f"Pipeline {side} démarré")
    ...
    def stop(self):
        ...
        for client in self.tcp_client_sockets:
            try:
                client.close()
            except:
                pass
